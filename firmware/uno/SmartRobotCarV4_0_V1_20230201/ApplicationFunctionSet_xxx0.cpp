...
static void ApplicationFunctionSet_SmartRobotCarLinearMotionControl(SmartRobotCarMotionControl direction, uint8_t directionRecord, uint8_t speed, uint8_t Kp, uint8_t UpperLimit)
{
  ...
  static uint8_t blockedReadings = 0;
...
{
  const uint8_t FORWARD_SPEED = 80;
  const uint8_t REVERSE_SPEED = 75;
  const uint8_t TURN_SPEED = 85;

  const uint16_t BLOCKED_DISTANCE_CM = 35;

  const uint8_t SENSOR_RIGHT = 30;
  const uint8_t SENSOR_CENTER = 90;
  const uint8_t SENSOR_LEFT = 150;
      ...
      ObstacleAvoidance_mode)
  {
    ...
    {
      ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
      AppServo.DeviceDriverSet_Servo_control(SENSOR_CENTER);
      firstRun = false;
    }

    AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(
        &frontDistance);

    // Zero means no echo or an object beyond the sensor range.
    boolean frontBlocked =
        (frontDistance > 0 &&
         frontDistance <= BLOCKED_DISTANCE_CM);
    ...
    else
    {
      ...
      // Move away from the obstacle before scanning.
      ApplicationFunctionSet_SmartRobotCarMotionControl(
      ...
      // Scan right.
      AppServo.DeviceDriverSet_Servo_control(SENSOR_RIGHT);
      AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(
          &rightDistance);

      // Scan left.
      AppServo.DeviceDriverSet_Servo_control(SENSOR_LEFT);
      AppULTRASONIC.DeviceDriverSet_ULTRASONIC_Get(
          &leftDistance);
      ...
      // Treat no echo as a clear, distant route.
      if (rightDistance == 0)
      {
        rightDistance = 150;
      }
      ...
      if (rightDistance <= BLOCKED_DISTANCE_CM &&
          leftDistance <= BLOCKED_DISTANCE_CM)
      {
        // Both routes are blocked: back farther and turn around.
        ApplicationFunctionSet_SmartRobotCarMotionControl(
            Backward, REVERSE_SPEED);
        delay_xxx(350);
